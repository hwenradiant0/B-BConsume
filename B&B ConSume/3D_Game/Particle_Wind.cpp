...
CParticle_Wind::CParticle_Wind(void)
{
	count = 0;
}
...
int CParticle_Wind::Create(LPDIRECT3DTEXTURE9 pTex)
{
	m_pTex = pTex;
	...
	if(m_bRender == true)
	{
		...
		if((m_last_frame - m_frame)>10)
		{
			for(int i=0 ; i<m_num ; i++)
			{
				...
				int	nIdxX	= m_Data[i].m_Img_Index % 14;
				int	nIdxY	= m_Data[i].m_Img_Index / 1;
				D3DXVECTOR2	uv00( (nIdxX+0)/14.f, (nIdxY+0)/1.f);
				D3DXVECTOR2	uv11( (nIdxX+1)/14.f, (nIdxY+1)/1.f);
			...
			for(int i=0 ; i<m_num ; i++)
			{
				...
				m_Data[i].m_Life = m_Data[i].m_Life - 0.02f;
...
void CParticle_Wind::SetRender(bool Render, D3DXVECTOR3 Pos, D3DXVECTOR3 Nor)
{
	m_bRender = Render;
	...
	if(m_bRender == true)
	{
		for(int i=0 ; i<m_num ; i++)
		{
			...
			fTheta = float(rand()%40); 
			fPhi = float(rand()%360);
			fSpdR = (rand()%101) * 0.1f;

 			if(Nor.x >= 0.9)
			{
				fTheta = (60 + float(rand()%40)); 
				fPhi = (80 + float(rand()%20));
			}
			else if(Nor.x <= -0.9)
			{
				fTheta = (60 + float(rand()%40)); 
				fPhi = (260 + float(rand()%20));
			}
			else if(Nor.z >= 0.9)
			{
				fTheta = (60 + float(rand()%40)); 
				fPhi = (350 + float(rand()%20));
			}
			else if(Nor.z <= -0.9)
			{
				fTheta = (60 + float(rand()%40)); 
				fPhi = (170 + float(rand()%20));
			}
			...
			m_Data[i].m_Opacity = D3DXCOLOR(1,1,1,1);
