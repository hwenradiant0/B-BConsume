...
CParticle_Send::CParticle_Send(void)
{
	count = 0;
}
...
int CParticle_Send::Create(LPDIRECT3DTEXTURE9 pTex)
{
	m_pTex = pTex;
...
void CParticle_Send::SetRender(bool Render, D3DXVECTOR3 Pos)
{
	m_bRender = Render;
	...
	if(m_bRender == true)
	{
		for(int i=0 ; i<m_num ; i++)
		{
			...
			m_Data[i].m_Init_Spe.y = -(10 + rand()%101) * 0.05f;
			m_Data[i].m_Init_Spe.z = 0;

			m_Data[i].m_Init_Pos.x = Pos.x + (-10 + rand()%20);
			m_Data[i].m_Init_Pos.y = Pos.y + (-10 + rand()%20);
			m_Data[i].m_Init_Pos.z = Pos.z + (-10 + rand()%20);
